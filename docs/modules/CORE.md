...
- 真实 Secret 不得进入 Git、Task、log、fixture 或 evidence；调用方同时遵守 `../SAFETY.md`。
- Core 还提供本机 CurrentUser DPAPI 的 opaque bytes 保护/解保护能力，供 Sheets 保存 OAuth grant。调用方只处理加密后的仓库外文件，不依赖 DPAPI 底层实现；读取或保护失败时 fail closed。
