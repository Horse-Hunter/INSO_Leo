...
- 一切设计、实现、测试、文档、流程和安全机制默认选择**能满足当前已确认目标的最简单方案**。
- 不为“以后可能需要”、理论完整性、架构漂亮或可扩展性提前增加抽象、框架、层级、状态、服务、配置、依赖或流程。
- 只有具体需求、已发生的问题、可复现的维护痛点或明确的高影响风险能够证明收益时，才增加复杂度；两个方案都能满足 Acceptance 时，选更少概念、更少文件、更少依赖、更短路径的方案。
- 安全同样遵循最低充分原则：保留能防止真实高影响事故的必要保护，优先简单的精确校验与 fail-closed；没有具体故障场景，不把额外 hardening 升级成 blocker。
- 发现方案开始“想太多”或持续衍生新机制时，默认先删减、合并或推迟，而不是继续叠加。
